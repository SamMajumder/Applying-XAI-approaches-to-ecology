...
## First we have to rename the columns ###
## Our barplot should the names of traits exactly as they are in the database ###
...
New_names <- Old_names %>% 
         str_replace("Leaf.C.N.ratio","Leaf C:N ratio") %>%
         str_replace_all("\\."," ") %>% 
         str_replace("Leaf curvedHeight maxWidth",
                     "Leaf curvedHeight max Width") %>% 
         str_replace("Trichomes density nonvein average",
                     "Trichomes density non vein average")
 
Data <- Data %>% rename_at(vars(Old_names), ~ New_names)


#######  Now we are applying our custom function "Missing" to the data 

columns_to_remove <- c("Ecoregions","individual_id","genotype_id",
                       "population_id","latitude","longitude","sitename")

Missing_data_results <- Missing(Data,y = columns_to_remove)
